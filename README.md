# Tide Admin B端脚手项目管理框架

道虽迩，不行不至；事虽小，不为不成。 -- 《荀子・修身》

## 简介

**AI 编程纪元已经开启，基于 Tide 项目开发，让 AI 助手“学习”更优雅的代码规范，“写出”更优质的代码。**
...
我们的愿景在于，当你将 Tide 系列项目应用到工作场景时，不仅仅是得到效率的提高，更可以得到舒适的开发体验，让更多开发者的编程工作多一点“甜”。

## 项目源码
GitHub: [wyh-admin](https://github.com/wangyonghao/wyh-admin)
...
在工作中，很多想法/设计受限于客户需求、开发工期，必须优先以交付为导向，但一些优秀的实践需要花时间持续进行沉淀，只要我没跳出这个圈子，我还是需要一直去做好程序归档。“种一棵树最好的时间是十年前，其次是现在”，最终，我选择在业余时间更加正视这件事，从头归档沉淀，从添加每一个依赖开始，我希望它能持续的迭代优化、演进，所以我把它命名为 **Tide（Continue New）**。并且这次我选择了开源，我希望它不仅仅能吸收我的需求和沉淀，而是依托开源协作模式，及时发现更多的问题，接受更多的可能性，沉淀更优秀的思考，设计。
...
- cmn 组件集合：针对 Spring 基础配置、通用解决方案及流行框架进行深度封装，改善你开发每个 Spring Boot Web 项目的体验（包含时间日期及枚举参数自动转换、默认线程池、跨域、加密、脱敏、限流、幂等、License、日志、异常及响应通用解决方案等）；
...
许多项目在开发或交付过程中需满足 Sonarqube 等质量指标，让你从一开始就站在高质量的起点。
...
- 角色管理：管理系统角色的功能权限及数据权限，包含新增、修改、删除、复制，移除成员等功能
...
> [!TIP]
> 更详细的流程，请查看在线文档[《快速开始》](https://wyhao.top/docs/admin/guide/quick-start.html)。

```
# 1.克隆本项目
git clone https://github.com/wangyonghao/wyh-admin.git
...
│  │  │  │  ├─ controller（通用 API）
│  │  │  │  ├─ job （定时任务）
│  │  │  │  └─ TideAdminApplication.java（Tide Admin 启动程序）
│  │  │  └─ resources
│  │  │     ├─ config（核心配置目录）
...
│  │  ├─ src
│  │  │  ├─ main
│  │  │  │  ├─ java/top.wyhao/extension/schedule
...
作为开源社区，我们的初衷是希望通过开源协作模式，提升技术透明度、放大集体智慧、共创优秀实践，源源不断地为企业级项目开发提供助力。

我们诚挚邀请广大社区用户为 Tide 项目贡献力量，包括但不限于 Issue 排查、测试验证、代码开发与重构等。每一份贡献，都是推动项目进步的重要力量（请查阅 [贡献指南](https://wyhao.top/about/contributing.html)）。欢迎各位感兴趣的小伙伴儿，[添加微信](https://wyhao.top/discussion.html) 讨论或认领任务。

### 分支说明

Tide 系列项目采用清晰的分支策略，确保开发与维护有序进行。提交 PR 前，请确认目标分支是否处于活跃维护状态，版本支持情况请查看 [更新日志#版本支持](https://wyhao.top/docs/admin/changelog/)。
...
3. 基于当前维护的分支（如 dev）创建新分支（如 feat/newFeature），请勿直接修改源分支（源分支仅做同步 Tide 最新代码用）
4. 在新分支上进行代码修改，完成后提交并 push 到您的远程仓库
5. 在开源平台上创建 pull request (PR)，选择正确的源分支和目标分支，按模板填写说明信息（参考 [已合并的 PR](https://github.com/wangyonghao/tide-admin-ui/pulls?q=is%3Apr+is%3Amerged) 可提高合并率）
...
> 2. 提交后端代码前请确保代码格式符合项目规范，避免不同 IDE 配置导致的格式差异 
...
感谢您对 Tide 开源项目的关注与支持！我们非常重视每一位用户的反馈和建议，这是推动项目不断进步的动力。 欢迎扫描下方二维码加入我们的官方交流群，与项目维护团队及其他大佬用户实时交流探讨。
...
<a href="https://github.com/wangyonghao/tide-admin/graphs/contributors">
  <img src="https://opencollective.com/tide-admin/contributors.svg?width=890&button=false" alt="contributors" />
</a>
...
- 遵循 <a href="https://github.com/wangyonghao/wyh-admin/blob/dev/LICENSE" target="_blank">Apache-2.0</a> 开源许可协议
- Copyright © 2022-present <a href="https://wyhao.top" target="_blank">wangyonghao</a>
