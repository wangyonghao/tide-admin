...
public class DashboardServiceImpl implements DashboardService {
    ...
    public DashboardOverviewCommonResp getOverviewPv() {
        DashboardOverviewCommonResp resp = operationLogMapper.selectDashboardOverviewPv();
        ...
        List<DashboardChartCommonResp> dataList = operationLogMapper.selectListDashboardAnalysisPv(last12MonthList);
        if (dataList.size() < 12) {
            // 填充缺失的数据
    ...
    public DashboardOverviewCommonResp getOverviewIp() {
        DashboardOverviewCommonResp resp = operationLogMapper.selectDashboardOverviewIp();
        resp.setGrowth(this.calcGrowthFromYesterday(resp.getToday(), resp.getYesterday()));
        List<String> last12MonthList = this.getLast12Months();
        List<DashboardChartCommonResp> dataList = operationLogMapper.selectListDashboardAnalysisIp(last12MonthList);
        if (dataList.size() < 12) {
            // 填充缺失的数据
            this.fillMissingDateData(last12MonthList, dataList);
        }
    ...
    public List<DashboardChartCommonResp> getAnalysisGeo() throws IOException {
        List<DashboardChartCommonResp> originList = operationLogMapper.selectListDashboardAnalysisGeo();
    ...
    public List<DashboardAccessTrendResp> listAccessTrend(Integer days) {
        ...
        List<DashboardAccessTrendResp> list = operationLogMapper.selectListDashboardAccessTrend(startTime, endTime);
        if (list.size() < days) {
    ...
    public List<DashboardChartCommonResp> getAnalysisTimeslot() {
        List<DashboardChartCommonResp> list = operationLogMapper.selectListDashboardAnalysisTimeslot();
        if (list.size() < 12) {
            // 获取所有时间段
    ...
    public List<DashboardChartCommonResp> getAnalysisModule() {
        return operationLogMapper.selectListDashboardAnalysisModule(10);
    }

    @Override
    public List<DashboardChartCommonResp> getAnalysisOs() {
        List<DashboardChartCommonResp> list = operationLogMapper.selectListDashboardAnalysisOs(4);
        return this.buildOtherPieChartData(list);
    }

    @Override
    public List<DashboardChartCommonResp> getAnalysisBrowser() {
        List<DashboardChartCommonResp> list = operationLogMapper.selectListDashboardAnalysisBrowser(4);
        return this.buildOtherPieChartData(list);
    }
    ...
    private List<DashboardChartCommonResp> buildOtherPieChartData(List<DashboardChartCommonResp> list) {
        Long totalCount = operationLogMapper.selectCount(Wrappers.emptyWrapper());
